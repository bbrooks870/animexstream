        ...
        >
          {top?.map((anime, i) => (
            <SwiperSlide key={i}>
              <ImageContainer
                style={{
                  '--bg-image': `url(${anime.images.jpg.large_image_url})`
                }}
              >
                <div className="absolute lg:w-[80%] 2xl:w-[62%] h-full  p-4">
                  <div className="flex flex-col items-start justify-center h-full">
                    <h1 className="text-2xl lg:text-4xl text-white drop-shadow-2xl font-black">
                      {anime.title}{' '}
                    </h1>
                    ...
                    <Description className="text-gray-300 px-2 mt-2 font-extralight">
                      {anime.synopsis}
                    </Description>
                    ...
                    <div className="flex justify-between gap-1 p-2">
                      <button
                        onClick={() => router.push(`/details/${anime.mal_id}`)}
                        className="py-2 px-4 bg-[#2227] text-gray-200 rounded-sm font-semibold flex  items-center justify-center gap-2  hover:bg-blue-800 hover:scale-105 transition-all ease-in-out"
                      >
                        <FaPlay size={16} />
